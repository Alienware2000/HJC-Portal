  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="mt-1.5 w-full h-11 rounded-lg border border-gray-300 px-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 focus:outline-none transition-all"
            {...register("email")}
          />
          {errors.email && <p className="text-[13px] text-red-600 mt-1">{errors.email.message}</p>}
        </div>
        ...
        <div>
          ...
          <input
            ...
            {...register("password")}
          />
          {errors.password && <p className="text-[13px] text-red-600 mt-1">{errors.password.message}</p>}
        </div>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 flex items-center justify-center gap-4 text-[13px]">
        ...
        <Link href={role === "admin" ? "/staff-login" : "/admin-login"} className="text-gray-400 hover:text-gray-600 transition-colors">
