...
export function Header({ role }: { role: Role }) {
  ...
  const roleDashboard = role === "board_member" ? "/member" : `/${role}`;
  const roleLabel = roleDashboard === "/member" ? "Overview" : roleDashboard.replace("/", "").charAt(0).toUpperCase() + roleDashboard.slice(2);
