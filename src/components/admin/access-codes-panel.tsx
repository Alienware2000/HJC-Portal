    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-white p-5 shadow-[0_0_0_1px_rgba(0,0,0,0.03),0_2px_4px_rgba(0,0,0,0.05),0_12px_24px_rgba(0,0,0,0.05)]">
        ...
        <div className="flex gap-2">
          ...
          />
          <button onClick={handleGenerate} disabled={isPending} className="h-10 px-4 rounded-lg bg-gray-900 text-white text-sm font-medium hover:bg-gray-800 active:bg-gray-950 focus-visible:ring-2 focus-visible:ring-ring/50 disabled:opacity-50 disabled:pointer-events-none transition-all flex items-center gap-1.5 shadow-sm shrink-0">
      ...
      <div className="flex gap-1.5">
        {(["all", "unused", "used"] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={`rounded-lg px-3 py-1.5 text-[13px] font-medium transition-all focus-visible:ring-2 focus-visible:ring-ring/50 ${filter === f ? "bg-gray-900 text-white shadow-sm" : "bg-gray-100 text-gray-500 hover:text-gray-700 hover:bg-gray-200"}`}>
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map((code) => (
              <tr key={code.id} className="hover:bg-gray-50/70 transition-colors">
                ...
                <td className="px-4 py-3.5 text-sm text-gray-700">{code.board_member_name}</td>
                ...
                <td className="px-4 py-3.5 text-right">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => handleCopy(code.code)} aria-label={`Copy code ${code.code}`} className="p-2 rounded-md text-gray-400 hover:text-gray-600 hover:bg-gray-100 focus-visible:ring-2 focus-visible:ring-ring/50 transition-all">
                      {copiedCode === code.code ? <Check className="h-3.5 w-3.5 text-emerald-500" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                    {!code.is_used && (
                      <button onClick={() => handleDelete(code.id, code.board_member_name)} aria-label={`Delete code for ${code.board_member_name}`} className="p-2 rounded-md text-gray-400 hover:text-red-500 hover:bg-red-50 focus-visible:ring-2 focus-visible:ring-ring/50 transition-all">
